...
# --- MODE: GABUNG FILE TT Type Site Down---
    ...
    if uploaded_files:
        with st.spinner("📦 Loading & Cleaning Data..."):
            ...
            if dataframes:
                ...
                # ✳️ Filter berdasarkan kolom TT Type jika ada
                if 'TT Type' in combined_df.columns:
                    ...
                    st.success(f"✅ Filter selesai! {len(filtered_df)} baris dengan 'TT Type' mengandung variasi 'Site Down' tanpa 'Non/Not'.")
                    st.dataframe(filtered_df)
                else:
                    filtered_df = combined_df
                    st.warning("⚠️ Kolom 'TT Type' tidak ditemukan. Data tidak difilter.")

                # ✅ Checkbox untuk pilih hasil ekspor
                st.markdown("Apakah ingin mengekspor data kolom 'TT Type' berisikan baris 'Site Down' saja atau semua data gabungan.")
                export_filtered = st.checkbox("🔘 Ekspor hanya kolom 'TT Type' berisikan baris 'Site Down'", value=True)
                data_to_export = filtered_df if export_filtered else combined_df

                # 🧾 Tombol download hasil ekspor
                if output_format == "Excel (.xlsx)":
                    # Ekspor sebagai file .xlsx langsung (tanpa ZIP)
                    ...
                    # Ekspor sebagai file CSV langsung
                    csv_buffer = data_to_export.to_csv(index=False).encode('utf-8')
                    ...
                    db_data = export_to_sqlite(data_to_export)
                    if db_data:
                        st.download_button(
...
if mode == "Dashboard":
    ...
    uploaded_file = st.file_uploader("Upload File Data (CSV, Excel, Parquet, Feather, atau SQLite DB)", 
